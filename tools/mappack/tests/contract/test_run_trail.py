...
class TestRunTrail(unittest.TestCase):
    ...
    def test_kilometre_markers_are_interpolated_bounded_and_drawn(self):
        self.assertIn("const MAX_KM_MARKERS = 50;", self.source)
        self.assertIn("const MAX_DRAWN_KM_MARKERS = 12;", self.source)
        self.assertIn("hidden var _kmLats as Array<Number>;", self.source)
        self.assertIn("hidden var _kmLons as Array<Number>;", self.source)
        self.assertIn("hidden var _kmNumbers as Array<Number>;", self.source)
        add = function_body(self.source, "add")
        self.assertIn("recordKilometres", add)
        record = function_body(self.source, "recordKilometres")
        self.assertIn("fraction", record)
        self.assertIn("MAX_KM_MARKERS", record)
        draw = function_body(self.source, "draw")
        self.assertIn("drawKilometreMarkers", draw)
        markers = function_body(self.source, "drawKilometreMarkers")
        self.assertIn("MAX_DRAWN_KM_MARKERS", markers)
        self.assertIn("dc.fillCircle", markers)
        self.assertIn("(_kmNumbers[i] / 2).format", markers)

    def test_half_kilometres_and_visibility_modes_are_supported(self):
        modes = read_repo("source/MarkerMode.mc")
        self.assertIn("const ALL = 0;", modes)
        self.assertIn("const KM_ONLY = 1;", modes)
        self.assertIn("const HIDDEN = 2;", modes)
        self.assertIn("500.0d", function_body(self.source, "recordKilometres"))
        markers = function_body(self.source, "drawKilometreMarkers")
        self.assertIn("markerMode == MarkerMode.HIDDEN", markers)
        self.assertIn("markerMode == MarkerMode.KM_ONLY", markers)
        self.assertIn("0xFF8800", markers)
